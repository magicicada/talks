...
\only<1>{
 ...
 \only<2>{Note: this is also called \textsc{Eulerian Trail} by Marino \& Silva.}
...
\begin{itemize}
\item \redout{Give up}
\item Approximate it - but some problems are hard to approximate, and sometimes an approximate answer isn't an answer
\item Throw a solver or a heuristic at it - often excellent in practice, but no guarantees
\item Accept an exponential in $n$ - fine until $n$ is the size of the cattle trade network
\end{itemize}

\bigskip

\textbf{Or: work out what it is about your instances that makes them easy, measure that, and pay the exponential only in the measurement.}
\end{frame}
...
\begin{frame}{When it doesn't work}
Not everything is FPT, and there is a theory of why not.

\begin{itemize}
\item \textbf{\textcolor{red}{W[1]-hard}}: no FPT algorithm unless the parameterised hierarchy collapses.  The standard example is \textsc{Clique} parameterised by solution size - we believe you cannot do better than roughly $n^{k}$.
\item \textbf{\textcolor{red}{para-NP-hard}}: the problem is already NP-hard for some \emph{fixed, constant} value of the parameter.  Then the parameter is simply the wrong one.
\end{itemize}

\bigskip

\textbf{Choosing badly is easy, and on temporal graphs it is easier still: several very natural parameters turn out to be para-NP-hard.}
\end{frame}
...
\begin{itemize}
    ...
    \end{itemize}
    \item \textbf{temporal clustering closedness}
